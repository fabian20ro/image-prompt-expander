...
def create_gallery(
    ...
    gallery_html = _build_gallery_html(prefix, cards_html, completed, total_images)
...
def generate_gallery_for_directory(prompts_dir: Path) -> Path:
    ...
    # Create gallery
    gallery_path = create_gallery(prompts_dir, prefix, prompts, images_per_prompt)
...
<head>
  ...
  <style>
    ...
    .card .prompt {{ padding: 12px; font-size: 13px; color: #aaa; max-height: 80px; overflow: hidden; }}
  </style>
</head>
<body>
  <h1>Gallery: {prefix}</h1>
